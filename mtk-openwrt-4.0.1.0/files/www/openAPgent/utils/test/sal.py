...
def sal_docker_download(method, request, pk):
  # For Python APIs
  if method == SAL_METHOD_LIST:
    return py_docker_download_list()

  if method == SAL_METHOD_RETRIEVE:
    return py_docker_download_retrieve(pk, 1)

  if method == SAL_METHOD_POST:
    return py_docker_download_post(request, pk)

  if method == SAL_METHOD_DETAIL_POST:
    return py_docker_download_detail_post(request, pk)

  if method == SAL_METHOD_DESTROY:
    return py_docker_download_destroy(request, pk)

  if method == SAL_METHOD_DETAIL_DESTROY:
    return py_docker_download_detail_destroy(request, pk)
...
def sal_container_creation(method, request, pk):
  # For Python APIs
  if method == SAL_METHOD_POST:
    return py_container_creation_post(request, pk)

  if method == SAL_METHOD_DETAIL_POST:
    return py_container_creation_detail_post(request, pk)
...
def sal_container_management(method, request, pk):
  # For Python APIs
  if method == SAL_METHOD_POST:
    return py_container_management_post(request, pk)

  if method == SAL_METHOD_DETAIL_POST:
    return py_container_management_detail_post(request, pk)
