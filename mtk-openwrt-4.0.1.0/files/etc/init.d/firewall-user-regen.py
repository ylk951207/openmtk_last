...
def iptables_generate_firewall_user_file(docker_lines):
        wline = ''
        ...
        with open (FIREWALL_USER_TEMP_FILE, "w") as wfile:
                wfile.write(wline)
                wfile.write(docker_lines)

        os.rename(FIREWALL_USER_TEMP_FILE, FIREWALL_USER_FILE)


docker_lines = iptables_get_docker_rule()
iptables_generate_firewall_user_file(docker_lines)
